...
const heroBanner = function ({ results: movieList }) 
{
 ...
 console.log(movieList);
    ...
    // fetch data for home page sections (top,upcoming,trending)

    for(const { title, path } of homePageSections) {
        let url = `https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1`;
        fetchdataFromServer(url, createMovieList, title);
    }
...
const createMovieList = function ({results: movieList}, title) {
    ...
    for (const movie of movieList) {
        const movieCard = createMovieCard(movie); //calledfrom movie_card js
